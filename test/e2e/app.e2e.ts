...
beforeAll(async () => {
  proc = spawn('pnpm', ['run', 'dev'], { stdio: 'pipe', cwd: process.cwd(), env: process.env, shell: true });
...
describe('E2E dev server checks', () => {
  it('serves known routes with 200 and HTML content', async () => {
    const urls = ['/', '/about', '/contact', '/works/1.body-navigation', '/consultancies/1.dr1-airlook'];
