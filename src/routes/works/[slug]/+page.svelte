...
<article>
		...
		<section class="pb-16 md:pb-20">
			<div class="content-width">
				...
				{#if data.item.meta.photocredits}
					<p class="mt-4 text-xs text-[var(--color-ink-subtle)]">Photos: {data.item.meta.photocredits}</p>
				{/if}
			</div>
		</section>
	{/if}
				...
				<aside class="mt-16 border-t border-[var(--color-border-subtle)] pt-8">
					<div class="grid gap-8 sm:grid-cols-3">
						{#if data.item.meta.materials}
							<div>
								<h4 class="mb-2 text-xs font-medium tracking-widest text-[var(--color-ink-subtle)] uppercase">Materials &amp; Equipment</h4>
								<p class="text-sm leading-relaxed text-[var(--color-ink-muted)]">{data.item.meta.materials}</p>
							</div>
						{/if}
						{#if data.item.meta.partners}
							<div>
								<h4 class="mb-2 text-xs font-medium tracking-widest text-[var(--color-ink-subtle)] uppercase">Partners</h4>
								<p class="text-sm leading-relaxed text-[var(--color-ink-muted)]">{data.item.meta.partners}</p>
							</div>
						{/if}
	...
	<nav class="border-t border-[var(--color-border-subtle)] py-8">
		<div class="narrow-width">
			<a href="/works" class="hover-line text-sm font-medium tracking-wide text-[var(--color-ink-muted)] uppercase transition-colors hover:text-[var(--color-ink)]">Back to Works</a>
		</div>
	</nav>
</article>
